...
# The main library consists of two functions:
# - getImage(row, column)
# - getBeadImage(row, column, color)
#
# These functions return the image that can be added to a label at
# the row and column of the pente board.  The getBeadImage returns
# the same underlying image with a bead played on it.
...
def getImage(row, column):
    ...
    if row < 0 or row > 19:
        print("Error - row must be 0 to 19")
        return 1

    if column < 0 or column > 19:
        print("Error - column must be 0 to 19")
        return 1
...
def getBeadImage(row, column, color):
    global images
    ...
    if color != "Blue" and color != "Red":
        print("Error - use either Blue or Red")
        return 1
    ...
    return images[__getKey(row, column) + color]
